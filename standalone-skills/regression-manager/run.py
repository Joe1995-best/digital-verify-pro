#!/usr/bin/env python3
"""regression-manager — Multi-run regression tracking with history"""
...
logger = get_logger("regression-manager")


def parse_args(argv=None):
    """Parse CLI arguments matching skill_spec.json interface.signature"""
    ap = argparse.ArgumentParser(description="Multi-run regression tracking with history")
...
def main(argv=None) -> int:
    ...
    if errors:
        ...
        write_result(status="fail", module="regression-manager", errors=errors,
    ...
    write_result(
        ...
        module=args.spec.stem if args.spec and args.spec.exists() else "regression-manager",
