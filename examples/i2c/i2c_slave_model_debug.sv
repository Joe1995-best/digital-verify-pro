// I2C Slave Model â€?for simulation only, not synthesizable
...
module i2c_slave (
  ...
  wire start_det   = sda_sync && !sda_prev && scl_sync;    // SDAâ†?while SCLâ†?
  wire stop_det    = !sda_sync && sda_prev && scl_sync;   // SDAâ†?while SCLâ†?
    ...
    if (!rstn) begin
      ...
      case (state_q)
        ...
        SlaveAddr: begin
          ...
          if (scl_rising) begin
            ...
            if (bit_cnt_q < 4'd7)
              ...
              // Check address match
              if ({shift_q[6:0], sda_sync} >> 1 == slave_addr) begin $display("SLAVE: Address matched! shift=%h, sda_sync=%b", shift_q, sda_sync); end
                addr_match_q <= 1.b1; $display("SLAVE: Setting addr_match");
              else
                addr_match_q <= 1'b0;
        ...
        SlaveAckAddr: begin
          ...
          if (addr_match_q) begin $display("SLAVE: ACKing address");
            if (!scl_sync) begin
              // SCL low: drive SDA low for ACK
              sda_drive_q <= 1'b0;
              sda_en_q    <= 1'b1;
            end else begin
              // SCL high: release SDA after sampling
              sda_en_q <= 1'b0;
              if (scl_rising && bit_cnt_q == 4'd0) begin
                bit_cnt_q <= 4'd8;
                if (read_mode_q) begin
                  shift_q <= rd0;  // load first read byte
                  state_q <= SlaveRead; $display("SLAVE: -> SlaveRead");
                end else begin
                  state_q <= SlaveWrite;
                end
              end
        ...
        SlaveRead: begin
          $display("SLAVE_READ enter: shift_q=%h, bit_cnt=%d", shift_q, bit_cnt_q);
          // Drive data on SCL edges â€?keep SDA driven through SCL high
          ...
          if (scl_rising && bit_cnt_q <= 1) begin
            // Byte done â€?release SDA, master drives ACK
            sda_en_q <= 1'b0;
